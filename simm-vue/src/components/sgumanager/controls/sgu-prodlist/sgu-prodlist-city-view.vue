    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-table-column width="150" label="佣金信息(元)" :key="Math.random()">
            <template slot-scope="scope">
              ...
              <div v-if="canShow(`isAgent`)">
                团长实得佣金：
                <b>{{ parseFloat( scope.row.commissionAmount * (1 - baseData.slbProfit) || 0 ).toFixed(2) }}</b>
              </div>
              <!-- 未审核通过(城市设置成开启销提分离)则显示 -->
              <div
                v-if=" baseData.citySaleLogisticsPart === 1 && sgu.processStatus !== 3 "
                class="text-theme"
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.stockSellableStock }}</span>
            </template>
          </el-table-column>
          <el-table-column
            width="100"
