...
export default {
  methods: {
     ...
     */
    canShow(prop) {
      var propConfig = editPropMap.get(prop);
      if (propConfig == null || propConfig.showValid == null) {
        return false;
      }
      return propConfig.showValid(this);
    },
    /**
     * 能否编辑
     * @param {*} prop
    ...
    canEdit(prop) {
      if (prop == null) {
        return canEditFunc(this);
      }
      var propConfig = editPropMap.get(prop);
      if (propConfig == null || propConfig.editValid == null) {
        return false;
      }
      return propConfig.editValid(this);
    },
    setSendDate(val, row) {
      if (val) {
