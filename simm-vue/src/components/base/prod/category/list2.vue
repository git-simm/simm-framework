<template id="list">
  <section class="content-section">
      ...
      <el-form :model="request" label-width="120px" class="demo-ruleForm">
        <el-col :span="8">
          <el-form-item label="二级类目名称">
            <el-input placeholder="请输入二级类目名称" v-model.trim="request.category_name"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
    ...
    <div class="search-table">
      <el-button type="primary" @click="addShow()" class="btn-default" v-if="idflag != 0">新增二级类目</el-button>
      <el-table border :data="infoData.data" style="width: 100%">
        <el-table-column prop="id" label="二级类目ID"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <router-link :to="'/base/prod/category/list3/' + scope.row.id">
              <el-button plain size="minier">三级分类</el-button>
            </router-link>
          </template>
        </el-table-column>
      </el-table>
      ...
      ></el-pagination>
      <router-link to="/base/prod/category/list">
        <el-button type="primary" class="btn-default">返回</el-button>
      </router-link>
    </div>
    <el-dialog title="新增二级类目" :visible.sync="addFlg">
      <el-form :model="addData" label-width="25%" :rules="rules" ref="addData">
        <el-form-item label="二级类目名称" prop="categoryName">
          <el-input placeholder="请输入二级类目名称" v-model.trim="addData.categoryName" auto-complete="off"></el-input>
        </el-form-item>
    ...
    <el-dialog title="编辑类目" :visible.sync="editFlg">
      <el-form :model="selected" label-width="25%" :rules="rules" ref="selected">
        ...
        <el-form-item>
          <h5 style="color: red;">
            注意：<br/>1、如更改，城市毛利率将会同步调整（不包含代理城市）；<br/>2、如毛利率发生变更，正在售卖、待上架、已下架与等待审核的SGU不受影响；
          </h5>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    addShow: function() {
      this.addData = {
        ...
        status: "1",
        afterTime: null,
        grossProfitRate:this.parentCategory.grossProfitRate
      };
      this.addFlg = true;
    },
