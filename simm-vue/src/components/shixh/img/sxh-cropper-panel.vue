...
<div class="sxh-cropper">
    ...
    <el-row>
        <el-col :span="14">
            <div class="cut">
                <vue-cropper ref="cropper" :img="option.img" :output-size="option.size" :output-type="option.outputType" :info="option.info" :info-true="option.infoTrue" :full="option.full" :fixed="option.fixed" :fixed-number="option.fixedNumber" :can-move="option.canMove" :can-move-box="option.canMoveBox" :fixed-box="option.fixedBox" :original="option.original" :auto-crop="option.autoCrop" :auto-crop-width="option.autoCropWidth" :auto-crop-height="option.autoCropHeight" :center-box="option.centerBox" @real-time="realTime" :high="option.high" mode="cover" :maxImgSize="option.maxImgSize"></vue-cropper>
            </div>
        </el-col>
        <el-col :span="10" class="preview">
            ...
            <p style="text-align:center;padding-top:10px;">
                预览图
                <el-tag size="mini" effect="plain" v-if="(option.fixedNumber||[]).length>=2">长宽比 {{option.fixedNumber[0]}}:{{option.fixedNumber[1]}}</el-tag>
                <el-tag size="mini" effect="plain" v-if="previews!=null">分辨率 {{cropInfo.width}} * {{cropInfo.height}}</el-tag>
            </p>
            <div v-html="info.show" style="overflow:auto;height: 170px;"></div>
        </el-col>
    </el-row>
